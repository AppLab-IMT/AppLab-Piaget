# tela cadastro de usuarios com role ADMINISTRADORES
...
def t09():
    ...
    root.title("NOVOS ADMINISTRADORES")
    ...
    label_title_page = tk.Label(root, text="NOVOS ADMINISTRADORES")
...
def transaction1(email_institucional, password, recovery_answer):
    role = "ADMINISTRADORES"
    db_path = os.path.join(os.path.dirname(__file__), "..", "data", "database.db")
    usuario_id = str(uuid4())
    usuario_id_administrador = str(uuid4())
    ...
    # Conectar-se ao banco de dados e executar a transação
        ...
        with sqlite3.connect(db_path) as conn:
            ...
            try:
                ...
                query_usuario_administrador = '''
                    INSERT INTO usuario_admin (id, usuario_id,  is_active)
                    VALUES (?, ?, ?)
                '''
                parameters_usuario_administrador = (usuario_id_administrador,usuario_id, 1)
                cursor.execute(query_usuario_administrador, parameters_usuario_administrador)
        ...
        return messagebox.showerror("Erro", "Email do ADMINISTRADOR inválido \n O email de administradores segue o padrão \n primeiroNome.ultimoSobrenome@jpiaget.com.br")
