...
def define_btn_e_texto(root, email):
    ...
    if LETTER == "A":
        return [primeiro_nome, ultimo_nome, "Jogar", lambda: switch_to_play(root), "Ranking", lambda: switch_to_ranking(root)]
    elif LETTER == "B":
        return [primeiro_nome, ultimo_nome, "Cadastrar", lambda: switch_to_register(root), "Ranking", lambda: switch_to_ranking(root)]
    elif LETTER == "C":
        return [primeiro_nome, ultimo_nome, "Questoes", lambda: switch_to_questions(root), "Ranking", lambda: switch_to_ranking(root)]
    else:
